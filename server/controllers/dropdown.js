...
import { getDropdownValuesService } from "../services/dropdownService.js";
  ...
  try {
    connectToDatabase(async (err, conn) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ success, data: err, message: errorMessage });
      }
      ...
      try {
        ...
        return res
          .status(200)
          .json({ success, data: results, message: infoMessage });
      } catch (queryErr) {
        logError(queryErr);
        return res
          .status(500)
          .json({ success: false, data: queryErr, message: "Query failed" });
      } finally {
      ...
      try {
        ...
        const result = Array.from(unitsMap.values()).map((unit) => ({
          ...
          files: unit.files.sort(
            (a, b) =>
              (a.SortingOrder || Number.MAX_SAFE_INTEGER) -
              (b.SortingOrder || Number.MAX_SAFE_INTEGER)
          ),
        }));
