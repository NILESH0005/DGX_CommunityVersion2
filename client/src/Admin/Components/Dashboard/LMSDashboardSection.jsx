...
const LMSDashboardSection = () => {
  ...
  const calculateStats = () => {
    ...
    const totalViews = moduleData.reduce((sum, module) => sum + module.totalViews, 0);
    const totalTimeSpent = moduleData.reduce((sum, module) => sum + module.totalTimeSpent, 0);
    
    // Calculate average rating (using placeholder values)
    const totalRatings = moduleData.reduce((sum, module) => sum + (module.avgRating || 0), 0);
    const avgRating = totalModules > 0 ? (totalRatings / totalModules).toFixed(1) : "0.0";

    // Find special modules
    const mostViewedModule = [...moduleData].sort((a, b) => b.totalViews - a.totalViews)[0];
    const leastViewedModule = [...moduleData].sort((a, b) => a.totalViews - b.totalViews)[0];
    const highestEngagementModule = [...moduleData].sort((a, b) => b.totalTimeSpent - a.totalTimeSpent)[0];
  ...
  const getTopPerformingModules = () => {
    return [...moduleData]
      .sort((a, b) => (b.totalViews * 2 + b.totalTimeSpent / 100) - (a.totalViews * 2 + a.totalTimeSpent / 100))
      .slice(0, 4);
  };
  ...
  const DashboardModuleCard = ({ module, index, highlight, metric }) => {
    const engagementPercentage = Math.min((module.totalTimeSpent / 3600) * 100, 100); // Cap at 100%
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          ...
          <div className="text-center">
            ...
            <p className="text-xs font-bold">{formatTime(module.totalTimeSpent)}</p>
            <p className="text-xs text-gray-500">Time</p>
          </div>
          <div className="text-center">
            <FaStar className="mx-auto text-gray-400 mb-1 text-sm" />
            <p className="text-xs font-bold">{module.avgRating?.toFixed(1) || "0.0"}</p>
          ...
          {stats.mostViewedModule && (
            ...
            >
              ...
              <p className="text-sm text-gray-600 mt-1">
                {stats.mostViewedModule.totalViews} views • {formatTime(stats.mostViewedModule.totalTimeSpent)}
              </p>
            </motion.div>
          )}
            ...
            <div className="space-y-4">
              {getModulesNeedingAttention().length > 0 ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm mt-2">No modules with low engagement detected</p>
                </div>
              )}
            </div>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-3 px-4 text-gray-600 font-medium">Module Name</th>
                  <th className="text-left py-3 px-4 text-gray-600 font-medium">Views</th>
                  <th className="text-left py-3 px-4 text-gray-600 font-medium">Time Spent</th>
                  <th className="text-left py-3 px-4 text-gray-600 font-medium">Engagement</th>
                  <th className="text-left py-3 px-4 text-gray-600 font-medium">Status</th>
                </tr>
              </thead>
              <tbody>
                {moduleData.slice(0, 8).map((module) => (
                  <tr key={module.moduleId} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center">
                        ...
                        <span className="font-medium">{formatTime(module.totalTimeSpent)}</span>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="w-24">
                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ 
                              width: `${Math.min((module.totalTimeSpent / 3600) * 100, 100)}%` 
                            }}
                          />
                        </div>
                        <p className="text-xs text-gray-500 mt-1">
                          {Math.min(Math.round((module.totalTimeSpent / 3600) * 100), 100)}%
                        </p>
                      </div>
                    </td>
                    <td className="py-3 px-4">
